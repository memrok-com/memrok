...
- **Operational eval ritual:** for injector/retrieval changes, run `npm run eval:injector` before and after the change; when a real bad header matters, convert it into a scrubbed fixture; before release, pair the eval run with package tests/build and a packaged-plugin smoke test instead of trusting only the dev-mounted plugin.
