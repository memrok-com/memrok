...
- **Use the inspection tooling before guessing.** `scripts/inspect-header.mjs`, `scripts/eval-sessions.mjs`, `scripts/inspect-reflection.mjs`, `scripts/manual-reflection.mjs`, `scripts/inspect-lifecycle.mjs`, `npm run eval:injector`, and `npm run eval:events` exist to make memory quality and curation behavior visible.
- **Operational eval ritual:** for injector/retrieval changes, run `npm run eval:injector` before and after the change; when a real bad header matters, inspect it with the session/header tooling, then convert it into a scrubbed fixture; before release, pair the eval run with package tests/build and a packaged-plugin smoke test instead of trusting only the dev-mounted plugin.
- **Runtime eval events are observation, not dumping.** If you enable `evalEvents`, keep the payload bounded and privacy-aware; these local records should help diagnosis and fixture creation, not become a shadow transcript archive.
