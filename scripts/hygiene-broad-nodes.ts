...
function findTopicAnchor(segments: string[]): string | null {
...
function assessNode(node: Node, nowMs: number): Assessment | null {
  ...
  if (broadScore < 0.45 && genericMetaScore < 0.3) return null;
  if (!weakAnchor && broadScore < 0.7) return null;
...
const assessments = store.queryNodes({ active: true })
  .map((node) => assessNode(node, Date.now()))
